(ns databricks.main
  "Kotodama WASM entrypoint for the Databricks clean-room actor (L4) — Clojure port.
  ...
  facts (`databricks.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "databricks")
(def tier "L4")
...
(def entity-specs
  [{:entity "Cluster"   :plural "clusters"   :id-prefix "databric_clu"
    :fields [:clusterName :state :sparkVersion :numWorkers] :required [:clusterName :state]
    :coerce {:numWorkers :int}        :refs {}}
   {:entity "Job"       :plural "jobs"       :id-prefix "databric_job"
    :fields [:jobId :creatorUserName :createdTime] :required [:creatorUserName :createdTime]
    :coerce {:jobId :int}             :refs {}}
   {:entity "Run"       :plural "runs"       :id-prefix "databric_run"
    :fields [:runId :jobId :state :runName :startTime] :required [:state :runName]
    :coerce {:runId :int :jobId :int}  :refs {}}
   {:entity "Warehouse" :plural "warehouses" :id-prefix "databric_war"
    :fields [:name :state :warehouseType :clusterSize] :required [:name :state]
    :coerce {}                        :refs {}}
   {:entity "ObjectInfo" :plural "objectinfos" :id-prefix "databric_obj"
    :fields [:path :objectType :language :size] :required [:path :objectType]
...
(defn emit-facts
  "EAVT facts for one record: {\"databricks.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "databricks-compat" :tier tier :entities entities} 200])
...
(defn start! [] :databricks-compat/ready)
